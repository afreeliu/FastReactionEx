...
public class HttpClientUtil {
	...
	private static HttpClientUtil m_HttpClientUtil;
	...
	public  HttpClientUtil getInstance() {
		if(m_HttpClientUtil == null){
			synchronized (HttpClientUtil.class) {
				if(m_HttpClientUtil == null){
					m_HttpClientUtil = new HttpClientUtil();
				}
			}
		}
		return m_HttpClientUtil;
	}
	 ...
	 */
	public String sendPostRequestByJava(String reqURL, Map<String, String> params){
	 ...
	 * @return 远程主机响应正文`HTTP状态码,如<code>"SUCCESS`200"</code><br>若通信过程中发生异常则返回"Failed`HTTP状态码",如<code>"Failed`500"</code>
	 */
	public String sendPostRequestByJava(String reqURL, String sendData){
	 ...
	 * @return
	 */
	public  final String sendHttpsRequestByPost(String url, Map<String, String> params) {
		String responseContent = null;
		HttpClient httpClient = new DefaultHttpClient();
		//创建TrustManager
		X509TrustManager xtm = new X509TrustManager() {
	 ...
	 * @throws Exception
	 */
	public JSONObject sendPostByJson(String url, JSONObject body) throws Exception {
		...
		try {
			...
			HttpEntity entity2 = new StringEntity(body.toString(), Consts.UTF_8);
	...
	public JSONObject sendPostByJson(String url, JSONObject body, JSONObject headers) throws Exception {
		CloseableHttpClient httpclient = HttpClients.custom().build();
		HttpPost post = null;
		String resData = null;
		CloseableHttpResponse result = null;
